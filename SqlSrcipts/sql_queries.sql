...
BEGIN
    DROP TABLE IF EXISTS temp_result;
        ...
        return query select tm.data_source, cat.description, em.emails_number, tm.proccesed, tm.not_processed
            ...
            left join (SELECT category_id, count(*) as emails_number
                        FROM public.emails
...
RETURNS TABLE(data_source character varying, category text, emails_number bigint, processed bigint, not_processed bigint)
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE
    ROWS 1000
...
BEGIN
        ...
        LOOP
            ...
            EXECUTE format('select count(*) from (select  distinct on (domain) domain from %s where  processed_1000 = TRUE and domain is not NULL) as _inner' , 'public.' || my_table_name) INTO processed;
            EXECUTE format('select count(*) from (select  distinct on (domain) domain from %s where  processed_1000 is not True and domain is not NULL) as _inner', 'public.' || my_table_name) INTO not_processed;
            INSERT INTO temp_result VALUES (my_table_name, processed, not_processed, counter);
        END LOOP;
        return query select tm.data_source, cat.description, em.emails_number, tm.proccesed, tm.not_processed
        	from temp_result as tm
            inner join  public.categories as cat on tm.category_id = cat.category_id
            left join (SELECT dom_cat.category_id, count(*) as emails_number
                        FROM public.emails as email
                        inner join domains_categories as dom_cat on email.domain=dom_cat.domain_name
                        group by dom_cat.category_id) as em on tm.category_id = em.category_id;
 END;
