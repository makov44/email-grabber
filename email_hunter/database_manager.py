...
@connector
def get_domains(categoty_id, cur):
...
from (SELECT distinct on (website) id, factual_id, name, address, address_extended, 
 ...
 as website, email, category_ids, category_lables, chaine_name, chain_id, hours, hours_display, existence, population 
...
def insert_emails(data, cur):
    ...
    lines = [(x['value'], x['type'], x["confidence"], reduce_sources(x['sources']), x['first_name'], x['last_name'],
              ...
              data['webmail'], data['pattern'], data['organization']) for x in lines]
    ...
    linkedin, twitter, phone_number, domain, webmail, pattern, organization) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
